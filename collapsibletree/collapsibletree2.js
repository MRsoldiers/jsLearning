...
function update(id, bool) {
	...
	if (bool) {//展开
		...
		var childrenX = nodeList[id].x + 100;
		var childrenMinY = nodeList[id].y - (children.length - 1) * (30 / 2);
		var childrenMaxY = nodeList[id].y + (children.length - 1) * (30 / 2);
		
		for (var i = 0; i < children.length; i++) {
			var childId = children[i].id;
			nodeList[childId].x = childrenX;
			nodeList[childId].y = childrenMinY + i * 30;
		}
		...
		for (var key in targetDepthDict.data) {//那字典还有用吗？
			...
			console.log([nodeShowingChildrenchildrenMinY - 15, nodeShowingChildrenchildrenMaxY + 15]);
		}
		...
		for (var i = 0; i < otherRange.length; i++) {
			if (otherRange[i][0] === yRange[0] && otherRange[i][1] === yRange[1]) continue;
				...
				for (var theId = id; theId < 11; theId++) {
					...
					while (stack.length != 0) {  
						...
						for (var i = children.length - 1; i >= 0; i--)
							if (nodeList[children[i].id].isShown)
								stack.push(children[i].id);						  
					}  
				}
				break;
			} 
			else if (isIntersect(yRange, otherRange[i]) == 1) {//情况1
			...
			else if (isIntersect(yRange, otherRange[i]) == 2) {//情况2
				var yMidLoc = (yRange[1] + yRange[0]) / 2;
				var otherMidLoc = (otherRange[i][1] + otherRange[i][0]) / 2;
				console.log(yMidLoc);
				console.log(otherMidLoc);
				if (yMidLoc < otherMidLoc) {//主动向上走，即增加 moveDistY （这里的moveDistY带负号
					var moveDistY = yRange[1] - otherRange[i][0];
					for (var theId = id; theId >= 1; theId--) {
						var stack = [];  
						stack.push(nodeList[theId].id);  
			...
			else if (isIntersect(yRange, otherRange[i]) == 3) {//情况3
				;
			}			
		}
	...
	for (var depth = 0; depth < 5; depth++) {
		for (var key in depthShownDict[depth].data) {
			if (depthShownDict[depth].data[key] !== null)
				drawCircle(nodeList[key], nodeList[key].x, nodeList[key].y)
		}
	}
}
